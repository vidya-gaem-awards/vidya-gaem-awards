...
$preferences = $mysql->real_escape_string(json_encode($preferences));

// Will be the steam ID if logged in, IP address if not. Not a great system
$_userID = $loggedIn ? '"'.$ID.'"' : "NULL";
$_code = isset($_SESSION['votingCode']) ? $mysql->real_escape_string($_SESSION['votingCode']) : "";
